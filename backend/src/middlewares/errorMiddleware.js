...
function getUploadClientError(err) {
  ...
  if (
    /multipart/i.test(raw) ||
    /unexpected end of form/i.test(raw) ||
    /unexpected field/i.test(raw)
  ) {
...
function errorHandler(err, _req, res, _next) {
  ...
  if (
    ...
    return res
      ...
      .json(
        createErrorResponse(
          'AWS signature mismatch. Verify AWS_ACCESS_KEY_ID/AWS_SECRET_ACCESS_KEY and ensure they belong to the same IAM key pair.',
        ),
      );
  }
  ...
  if (err?.name === 'AccessDenied' || err?.Code === 'AccessDenied') {
    return res
      ...
      .json(
        createErrorResponse(
          'AWS denied s3:PutObject for this IAM user. Attach an IAM policy that allows PutObject on this bucket/prefix.',
        ),
      );
  }
