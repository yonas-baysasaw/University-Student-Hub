...
export default function AdminInstructors() {
  ...
  const load = useCallback(async (pageNum) => {
    ...
    try {
      const res = await fetch(
        `/api/admin/users?accountType=instructor&page=${pageNum}&limit=20`,
        { credentials: 'include' },
      );
      const data = await readJsonOrThrow(res, 'Could not load instructors');
      setUsers(data.users ?? []);
  ...
  const toggleSuspended = async (u) => {
    ...
    try {
      ...
      await readJsonOrThrow(res, 'Update failed');
      toast.success(
        !u.instructorPostingSuspended
          ? 'Instructor restricted to read-only'
          : 'Instructor restrictions cleared',
      );
      await load(page);
    } catch (e) {
      toast.error(e?.message || 'Update failed');
    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-600 dark:text-slate-400">
          Toggle read-only lock (no writes anywhere on the platform, including chat).
        </p>
      </div>
      ...
      {loading ? (
        ...
        <>
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                  {users.length === 0 ? (
                    <tr>
                      ...
                      >
                        No instructor accounts match (add instructors by
                        editing a student on the Students page).
                      </td>
                    </tr>
                  ) : (
                    users.map((u) => (
                      <tr key={u.id}>
                        ...
                        <td className="px-4 py-2.5">
                          <label className="inline-flex cursor-pointer items-center gap-2">
                            <input
                              type="checkbox"
                              checked={u.instructorPostingSuspended}
                              disabled={busyId === u.id}
                              onChange={() => toggleSuspended(u)}
          ...
          <div className="flex flex-wrap items-center justify-between gap-2 text-xs text-slate-500">
            <span>
              {total} instructor(s) · page {page}/{totalPages}
            </span>
