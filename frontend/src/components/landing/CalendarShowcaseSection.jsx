...
function CalendarShowcaseSection() {
  ...
  return (
    <section
      id="calendar"
      className="scroll-mt-24 bg-slate-950 px-4 py-16 text-slate-100 md:px-6 md:py-24"
    ...
    >
      <div className="mx-auto max-w-6xl">
        ...
        <div className="grid gap-8 lg:grid-cols-[1fr_16rem]">
          <motion.div
            {...fadeUp(reduced, 0.1)}
            className="overflow-hidden rounded-2xl border border-slate-800 bg-slate-900/80 p-4 shadow-2xl md:p-6"
          >
            <div className="mb-4 flex items-center justify-between">
              <p className="font-display font-bold">April 2026</p>
            ...
            >
              {CALENDAR_DAYS.map((cell) => (
                <motion.div
                  key={cell.key}
                  {...staggerItem(reduced)}
                  className={`relative flex min-h-[2.75rem] flex-col items-center justify-start rounded-lg p-1 text-[11px] ${
                    cell.label
                      ? cell.today
                ...
                >
                  {cell.label ? (
                    <>
                      <span className={cell.today ? 'font-bold text-cyan-300' : ''}>
                        {cell.label}
                      </span>
                      {cell.today ? (
                        <motion.span
          ...
          <motion.div {...fadeUp(reduced, 0.15)} className="flex flex-col gap-3">
            ...
            {UPCOMING.map((item) => (
              ...
              >
                ...
                <p className="mt-1 font-mono text-[11px] text-cyan-400">{item.date}</p>
              </motion.div>
            ))}
          </motion.div>
        </div>
