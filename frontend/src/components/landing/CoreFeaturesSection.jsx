import { Bell, CalendarRange, Library, Search, Sparkles } from 'lucide-react';
import { motion, useReducedMotion } from 'framer-motion';
import {
  fadeUp,
  hoverLift,
  staggerContainer,
  staggerItem,
  useLandingMotion,
} from './landingMotion.js';

function VerifiedBadge() {
  const reduced = useReducedMotion();
  return (
    <motion.span
      className="inline-flex items-center gap-1 rounded-full bg-emerald-500/15 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-emerald-800 dark:text-emerald-300"
      animate={reduced ? undefined : { scale: [1, 1.05, 1] }}
      transition={
        reduced
          ? undefined
          : { duration: 2.5, repeat: Infinity, ease: 'easeInOut' }
      }
    >
      Verified
    </motion.span>
  );
}

const cards = [
  {
    id: 'announcements',
    icon: Bell,
    title: 'Verified announcements',
    body: 'Official posts from instructors and departments. Prioritize what matters with trusted, labeled updates.',
    preview: 'announcements',
  },
  {
    id: 'schedule',
    icon: CalendarRange,
    title: 'Dynamic schedule viewer',
    body: "See weekly patterns and today's slots at a glance—with clear time blocks and reminders.",
    preview: 'schedule',
  },
  {
    id: 'library',
    icon: Library,
    title: 'Structured digital library',
    body: 'Search and filter materials by type—notes, books, and assignment bundles in one place.',
    preview: 'library',
  },
  {
    id: 'liquai',
    icon: Sparkles,
    title: 'LiquAI study tools',
    body: 'AI study buddy and exam practice woven into the hub — no switching between apps.',
    preview: 'liquai',
  },
];

function CardPreview({ type }) {
  if (type === 'announcements') {
    return (
      <ul className="mt-4 space-y-2 border-t border-slate-100 pt-4 text-xs dark:border-slate-700">
        <li className="flex items-start justify-between gap-2">
          <span className="font-medium text-slate-800 dark:text-slate-200">
            Exam timetable released
          </span>
          <VerifiedBadge />
        </li>
        <li className="flex items-center justify-between text-slate-500 dark:text-slate-400">
          <span>Registration window</span>
          <span className="font-mono text-[11px]">Apr 28</span>
        </li>
        <li className="flex items-center justify-between text-slate-500 dark:text-slate-400">
          <span>Lab safety briefing</span>
          <span className="font-mono text-[11px]">Apr 29</span>
        </li>
      </ul>
    );
  }
  if (type === 'schedule') {
    return (
      <div className="mt-4 rounded-xl border border-slate-200 bg-slate-50/90 p-3 dark:border-slate-600 dark:bg-slate-900/50">
        <p className="font-display text-[11px] font-bold uppercase tracking-wide text-cyan-800 dark:text-cyan-300">
          Today&apos;s classes
        </p>
        <div className="mt-3 space-y-2">
          {[
            { t: '09:00–10:30', dot: true },
            { t: '11:00–12:30', dot: false },
            { t: '14:00–15:30', dot: true },
          ].map((row, i) => (
            <div
              key={i}
              className="flex items-center justify-between rounded-lg bg-white px-2 py-1.5 text-[11px] dark:bg-slate-800"
            >
              <span className="flex items-center gap-2 text-slate-700 dark:text-slate-300">
                {row.dot ? (
                  <motion.span
                    className="h-2 w-2 rounded-full bg-cyan-500"
                    animate={{ opacity: [1, 0.35, 1] }}
                    transition={{ duration: 1.6, repeat: Infinity }}
                    aria-label="Reminder"
                  />
                ) : (
                  <span className="w-2" aria-hidden />
                )}
                Section {String.fromCharCode(65 + i)}
              </span>
              <span className="font-mono font-semibold text-slate-600 dark:text-slate-400">
                {row.t}
              </span>
            </div>
          ))}
        </div>
      </div>
    );
  }
  if (type === 'liquai') {
    return (
      <div className="mt-4 rounded-xl border border-violet-200/60 bg-violet-50/40 p-3 dark:border-violet-800/40 dark:bg-violet-950/20">
        <p className="text-[11px] text-slate-600 dark:text-slate-400">
          <span className="font-semibold text-violet-700 dark:text-violet-300">
            LiquAI:
          </span>{' '}
          Merge sort is O(n log n) because each level does linear work…
        </p>
        <div className="mt-3 flex gap-2">
          {['Study Buddy', 'Exam Practice'].map((chip) => (
            <span
              key={chip}
              className="rounded-full border border-violet-200 bg-white px-2.5 py-1 text-[10px] font-semibold text-violet-800 dark:border-violet-700 dark:bg-slate-900 dark:text-violet-200"
            >
              {chip}
            </span>
          ))}
        </div>
      </div>
    );
  }
  return (
    <div className="mt-4 rounded-xl border border-slate-200 bg-white p-3 dark:border-slate-600 dark:bg-slate-900/40">
      <div className="flex items-center gap-2 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-[11px] dark:border-slate-600 dark:bg-slate-800/80">
        <Search className="h-3.5 w-3.5 shrink-0 text-slate-400" aria-hidden />
        <span className="text-slate-500">Search courses, titles…</span>
      </div>
      <div className="mt-3 flex flex-wrap gap-2">
        {['Notes', 'Books', 'Assignments'].map((c) => (
          <span
            key={c}
            className="rounded-full border border-slate-200 bg-slate-50 px-3 py-1 text-[11px] font-semibold text-slate-700 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-300"
          >
            {c}
          </span>
        ))}
      </div>
    </div>
  );
}

function CoreFeaturesSection() {
  const { reduced } = useLandingMotion();

  return (
    <section
      id="features"
      className="relative scroll-mt-24 px-4 py-12 md:px-6 md:py-16"
      aria-labelledby="features-heading"
    >
      <div className="relative z-[1] mx-auto max-w-6xl">
        <motion.div {...fadeUp(reduced)} className="mx-auto mb-10 max-w-2xl text-center">
          <h2
            id="features-heading"
            className="font-display text-2xl font-bold text-slate-900 dark:text-slate-50 md:text-3xl"
          >
            Core modules
          </h2>
          <p className="mt-2 text-sm text-slate-600 dark:text-slate-400">
            Structured tools you use daily—organized for clarity.
          </p>
        </motion.div>

        <motion.div
          className="grid gap-6 sm:grid-cols-2 md:gap-5 lg:grid-cols-4"
          {...staggerContainer(reduced, 0.1)}
        >
          {cards.map((card) => {
            const Icon = card.icon;
            return (
              <motion.article
                key={card.id}
                id={card.id === 'liquai' ? undefined : card.id}
                {...staggerItem(reduced)}
                {...hoverLift(reduced)}
                className={`panel-card scroll-mt-24 flex flex-col rounded-2xl p-6 transition-shadow duration-300 hover:shadow-lg dark:hover:border-cyan-900/60 ${
                  card.id === 'liquai' ? 'border-violet-200/40 dark:border-violet-900/30' : ''
                }`}
              >
                <div
                  className={`flex h-11 w-11 items-center justify-center rounded-xl ${
                    card.id === 'liquai'
                      ? 'bg-violet-500/12 text-violet-700 dark:text-violet-300'
                      : 'bg-cyan-500/15 text-cyan-700 dark:text-cyan-400'
                  }`}
                >
                  <Icon className="h-5 w-5" aria-hidden />
                </div>
                <h3 className="mt-4 font-display text-lg font-bold text-slate-900 dark:text-slate-50">
                  {card.title}
                </h3>
                <p className="mt-2 flex-1 text-sm leading-relaxed text-slate-600 dark:text-slate-400">
                  {card.body}
                </p>
                <CardPreview type={card.preview} />
              </motion.article>
            );
          })}
        </motion.div>
      </div>
    </section>
  );
}

export default CoreFeaturesSection;
